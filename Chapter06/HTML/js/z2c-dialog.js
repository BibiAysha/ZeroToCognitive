...
//customDialog['repeat'] = {type: 'talk', next: 'repeatListen', message: "I didn't get that could you repeat yourself?"};
//customDialog['repeatListen'] = {type: 'listen', next: 'repeatDetails'};
//customDialog['repeatDetails'] = {type: 'process', next: 'getClassificationText'};
  ...
  switch (step) {
    ...
    case 'actionSelect': // check if classification requested. if not, return to "how can I help you"
      ...
      if((str=="classify") || (str=="classified") || (str=="classifier"))
       ...
       else if(!(str=="classify") || (str=="classified") || (str=="classifier")) {
          talkToMe(dialog_target, "I didn't get that could you repeat yourself");
		  incrementStep(customDialog[step].getClassificationText);
		  //incrementStep(customDialog[step].repeat);
          nextStep(); 
		  }
      break;
    ...
    case 'classifyDetails': // listen for positive response.
      var str = trimStrip($("#chat_"+getCookieValue("prevstep"))[0].innerText).toLowerCase();
      if((str=="yes") || (str=="yup") || (str=="yeah" || str=="sure"))
        {
          incrementStep(customDialog[step].next);
          setModal(industryPage, displayNLC, industryTable, nlc_classes); 
		  }
		  
	else if((str=="no") || (str=="nope")){
		  alert("If you don't want my help, google your own problems");	
